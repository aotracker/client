...
interface PageProps {
  params: Promise<{ region: string; guildAId: string; guildBId: string }>;
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { region, guildAId, guildBId } = await params;
  ...
  const [guildA, guildB] = await Promise.all([
    getGuildByAlbionId(albionRegion, guildAId),
    getGuildByAlbionId(albionRegion, guildBId),
  ]);

  if (!guildA || !guildB) return notFoundMetadata();
  ...
  return buildPageMetadata({
    ...
    canonicalPath: `/feud/${region}/${guildAId}/${guildBId}`,
  });
}
